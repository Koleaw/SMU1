...
export const getSectionCategories = (snapshot: CatalogV2Snapshot, sectionSlug: string) =>
  snapshot.categories
    .filter((item) => item.parentSectionSlug === sectionSlug && item.showInSectionGrid)
    .sort((a, b) => (a.order - b.order) || a.title.localeCompare(b.title, 'ru'));

export const getActiveSectionCategories = (snapshot: CatalogV2Snapshot, sectionSlug: string) =>
  snapshot.categories
...
export const createCatalogV2Links = (
  ...
  const sectionCategories = getActiveSectionCategories(snapshot, sectionSlug);
