...
## Деплой на хостинг из GitHub

1. Подключить репозиторий GitHub к хостингу.
2. В настройках сборки указать команду `npm install && npm run build`.
3. В качестве папки публикации указать `dist`.
4. Для основного домена задать `SITE_URL`; `BASE_PATH` обычно должен быть `/`.

Локальная админка после сохранения делает `git push`, а подключенный хостинг запускает автодеплой по новому commit.
