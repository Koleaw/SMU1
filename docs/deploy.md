...
- требует `PRODUCTION_DEPLOY_ENABLED=true` и `SITE_URL`;
- сейчас, пока боевой режим не включен или `SITE_URL` не задан, админка показывает: «Боевой сайт пока не настроен. Используйте тестовую публикацию.» и не пушит в `main`.
...
- push в `main`/`master` -> `production` только при `PRODUCTION_DEPLOY_ENABLED=true`;
- push в `main`/`master` без `PRODUCTION_DEPLOY_ENABLED=true` -> `DEPLOY_TARGET=test`, `deploy_kind=check-only`, без production deployment;
- push в `preview`/`develop` -> `test`;
- ручной запуск `workflow_dispatch` -> выбранный `deploy_target`, но production разрешен только при `PRODUCTION_DEPLOY_ENABLED=true` и заданном `SITE_URL`.
...
- заранее проверяет `PRODUCTION_DEPLOY_ENABLED=true`;
...
Для `check-only` на `main`/`master`, пока production выключен:

- выводит сообщение `Production deploy is disabled. Set PRODUCTION_DEPLOY_ENABLED=true and SITE_URL to enable production deployment.`;
- запускает `npm run check` и `npm run build` с `TEST_SITE_URL` или GitHub Pages fallback;
- не запускает production deploy и не требует `SITE_URL`.
...
3. Задать repository variable `PRODUCTION_DEPLOY_ENABLED=true`.
4. Оставить `BASE_PATH=/`, если сайт открыт с корня домена.
5. Проверить production workflow вручную через `workflow_dispatch` с `deploy_target=production`.
6. После успешной проверки использовать кнопку «Опубликовать на сайт».
...
- event;
- выбранный target;
- detected `DEPLOY_TARGET`;
- `PRODUCTION_DEPLOY_ENABLED`;
- `SITE_URL configured`;
- `productionCheckRan`;
- почему workflow выбрал test или production;
